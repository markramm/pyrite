...
def setup(tmp_path):
    ...
    svc.create_entry(
        "test", "e1", "Event 1", "timeline_event", date="2025-01-01", actors=["Donald Trump", "FBI"]
    )
    svc.create_entry(
        "test",
        "e2",
    ...
    svc.create_entry(
        "test", "e3", "Event 3", "timeline_event", date="2025-03-01", actors=["FBI", "Pete Hegseth"]
    )
...
class TestExtractActors:
    ...
    def test_computes_importance(self, setup):
        ...
        assert (
            result["actors"]["Donald Trump"]["importance"]
            >= result["actors"]["Elon Musk"]["importance"]
        )
