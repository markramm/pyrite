<script lang="ts">
	...
	const activeKB = $derived(kb ?? kbStore.activeKB ?? undefined);
	const waitingForKB = $derived(
		!activeKB && (kbStore.loading || !kbStore.initialized)
	);
	const waitingForEntries = $derived(
		!!activeKB && (!entryStore.initialized || entryStore.listKB !== activeKB)
	);
	...
	function goToPage(page: number) {
		...
		entryStore.loadList({ kb: activeKB, offset });
	}
</script>

{#if !activeKB && kbStore.error}
	<ErrorState message={kbStore.error} onretry={() => kbStore.load()} />
{:else if entryStore.loading || waitingForKB || waitingForEntries}
	<SkeletonLoader variant="cards" lines={5} />
{:else if entryStore.error}
	<ErrorState message={entryStore.error} onretry={() => entryStore.loadList({ kb: activeKB })} />
