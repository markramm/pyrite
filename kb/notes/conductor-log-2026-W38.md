...
## Tick 2026-09-19T20:15Z (budgets: 4 suite slots / 2 armed PRs)

Load 3.1, 66% free, dev green (293672a — **#171 merged** by the maintainer, squash; #151's read half is in). With #171 landed the hold on our merges is lifted: **#163 and #160 rebased and armed** (the PR budget's two). Slots: two workers (#180 and #161 redispatches, both opus, both at `-n 4`) + one review suite = 3 of 4.

**Reviews since the last tick (CI green heads were not re-run, per the maintainer):** #163 ready — folded in the #159 fix (frontend test files count for the fix-needs-a-test rule; red first, 4 cases) since this PR is what enforces the rule in CI. #160 ready — the PR gate never runs Playwright, so the two specs were run once here: 21 passed. #145 rebased onto dev (one CHANGELOG stop), four `PT` lint hits from dev's newer rules fixed, pushed at 04dd989, gate green; cold read pending. #180 sent back: the cold read found, and the host reproduced, that `_resolve_kb_name` is first-wins — a request naming a readable KB in one place and a private KB in another is checked against the first and served from the second; the query-before-path ordering is pre-existing on dev (`/api/kbs/{name}/orient?kb=<readable>` answers for a private KB). Everything below the resolver held. Redispatched with the resolver contract (check every named KB), `/kbs/{name}/changes` scoped now, a structural check that the resolver looks where each handler reads, and the load-sensitivity of the new test files to fix. #161 sent back: five reproduced findings — `pull`/`push` now answer with a canned "see the server log" for almost every real failure, the no-remote case is mislabelled `REPO_NOT_FOUND`, the path regex eats URLs (`http<path>`) and leaks a directory name around a space, `sync`'s nested errors bypass `_error_detail`; plus the headline test passes with the sanitiser stubbed out. #140: second cold read asks for three fixes (nested fences still truncate notes; a stale green can release while an untimestamped rerun is in progress; the composed-command fixture misses step e's already-reopened branch) — **the same theme redispatched twice trips the breaker**, so it is on the maintainer's desk with a recommendation that the conductor makes the small fixes itself at review.

**New outside PR #184** (berkay-byte, first-time; fixes #179, the `?fields=` bugs filed this morning as a good first issue — 5 h from label to PR; +28/−4; CI `action_required`): in review now — suite at `-n 4` in the third slot, cold read out. Takes were agreed with the maintainer before posting on the last four outside PRs; same here.

## Tick 2026-09-19T20:27Z (reviews land: #184, #145, #161 second pass)

Load peaked at 8.6 while the #180 worker ran its suite; the conductor's #161 suite was queued behind a load gate (< 7) rather than started — the first time the budget's load rule bit, and it worked. **#184** (outside, first-time): conductor half clean (lint at CI scope, `-n 4` 4324/0, verify-red red on both tests); cold read could not break the fix — MCP-identical over 18 edge inputs, no scoping change, nothing `json.dumps` can trip on. Three asks drafted (a `Query` description saying the projection is not `EntryResponse`-shaped, a CHANGELOG line, the identity pair asserted on every result + one unknown-field case) and two follow-ups on our side (the CLI drops the identity pair too — reproduced live on the maintainer's KB; one shared `_project_fields` once #174 lands). Take handed to the maintainer, nothing posted. **#145**: second cold read, two findings reproduced — above sqlite-vec's 4096 cap a filtered semantic search silently returns nothing (needle at 4150 → `[]`, warnings empty; the maintainer's 18,909-entry index is past the cap), and the >4096 test cannot see it (needle always rank 1). Two smaller ones (`exclude_none` reshapes every result row; the archived exclusion trips the capability gate on unfiltered queries). Review appended to the PR. **Breaker: second redispatch of the same theme → on the maintainer's desk** with the recommendation that the conductor makes the four fixes at review. **#161** second pass reported: all five findings closed red-first, dead `clone_private_repo` deleted, no-network clone guard, one deliberate deviation (subscribe cannot depend on redaction — moved to fork/pr); report appended; second cold read out; suite queued. #173 is DIRTY after #171's squash (CHANGELOG + `models/base.py`); nudge drafted for the maintainer's agreement. Worktrees: `review-pr-184`, `review-pr-161`, `review-pr-145` (conductor's); the cold reads removed their own.

## Tick 2026-09-19T20:34Z (scheduled)

**Health:** `dev` at 2823e68 — **#163 merged** (CI parity: extensions linted, fix-needs-a-test on PR ranges, frontend test files count; closes #159), run in progress; #160 merged earlier this hour (6d3a45f). Load 3.4, 64% free. #164 closed by the maintainer (superseded by #173). Worktrees for #163/#160 removed, five stale merged branches deleted. Zero loop PRs armed: every remaining loop branch is on the maintainer's desk.

**Absorb:** nothing new — #180's worker (resolver contract) has not pushed since 1ce02e4. Since the last scheduled tick the review lane finished three: **#184** (outside) — take ready, three asks, awaiting agreement to post; **#145** — held, sqlite-vec 4096-cap silent under-return reproduced, one prior redispatch → breaker; **#161** — held, regex over-/under-redacts reproduced (`file://`, UNC, prose swallowed, guard argv), one prior redispatch → breaker. #140 was already on the desk. Recommendation standing with the maintainer: authorise conductor-made review-stage fixes when the breaker trips and the fix list is bounded — three branches, all small lists — rather than a third Opus pass each.

**Choose/dispatch:** none. Stop condition: the maintainer's queue (four decisions, six outside PRs awaiting merge/nudge) is longer than the agents'. Groom lane dispatched instead (read-only, no slot): architect grooming priorities 2 (extension base-field conformance) and 3 (relabel the three unused extensions as examples) into items with `## Groom` sections on this branch, plus a note of ready-to-file follow-up issue texts from today's reviews (CLI identity pair, shared `_project_fields`, sqlite-vec exact prefilter, repo success-path paths, `configure_logging` never called) — held for the maintainer's agreement before filing. Good-first-issue pool: 14 open.

**Needs the maintainer:** (1) #184 — post the agreed take (merge after 3 changes); (2) #173 — post the rebase nudge; (3) #140/#145/#161 — conductor fixes at review, or redispatch; (4) #177/#175/#174/#166 — agreed messages posted earlier, awaiting the contributors; merges are the maintainer's.

## Tick 2026-09-19T20:44Z (groom returned; dispatch)

Groom lane landed (c1045aa): priorities 2 and 3 have items with `## Groom` sections, and the follow-ups note holds five issue texts (held for the maintainer). The architect reproduced priority 2 from the code: six extension classes build `cls(...)` by hand and drop `aliases`, `importance`, `_schema_version` — `importance: 9` comes back 5 and is **written back as 5**; `extra_frontmatter` cannot rescue them because all three are in `_BASE_CONSUMED_KEYS`, which is why the round-trip safety net is blind here. Scope grew honestly: five hand-rolled copies of `_base_kwargs` (cascade, journalism, software-kb, `models/task.py`), each drifting differently — the theme is "one function", not "three extensions". Two corrections to the brief: the three extensions have no README at all (`docs/plugins.md`'s "Full docs" links land on a directory listing), and follow-up 5 was wrong as filed — `configure_logging` is called, but `pyrite/logging.py` is never imported anywhere, and its import-time handler install is itself a library smell. The architect also hit `pyrite create -t note` silently creating an `adr` — same class as `bug-create-silently-accepts-undeclared-types`; added to the follow-ups.

**Dispatched** (both Sonnet, both claimed on their branches with the item as the first commit): **#187** `fix/extension-base-field-conformance` — with the conductor's refinement that the conformance test asserts nothing about undeclared-key placement or added timestamps (#175/#173 own those), so it is true whichever order lands, and that the four in-tree copies are not deferred (a PR fixing three and leaving four drifting is a fragment). **#188** `fix/relabel-example-plugins` — wording only, three new READMEs, grep gate as evidence, no `.py` or test edits. Disjoint footprints from each other and from #180. Slots: #180 worker + two Sonnet workers = 3 of 4; load 3.2. Sequencing judgement, recorded: the architect put priority 2 behind #175 for the test's sake; the refinement above removes that dependency, and waiting days on an outside PR for a maintainer-ranked priority was the worse default. #186 (links.py secondary-KB params, part 2) was filed by the #180 worker.

## Tick 2026-09-19T20:57Z (scheduled)

**Health:** `dev` 2823e68 green (the one red run on it is the Dependabot Updates workflow, not CI — noted, not a breaker). Load peaked at 14.9 with three worker suites overlapping — memory stayed at 77% free, so the `-n 4` rule is holding the OOM line; the load rule queued the conductor's own suites behind a `< 7` gate. `dev`'s `extensions/` is ruff-clean after #163; branches based before it show 46 errors locally, which the rebase will erase — not a finding against any of them.

**Outside:** #173 rebased by the author to 1e06f97 (gate green — no longer first-time) with the four requested changes; re-review started: review worktree, suite queued, cold read out (`__setattr__` flag mechanics, `_timestamp_same_instant` edge cases, every `save` caller for `touch_updated_at`, the extension classes #187 is touching). #175/#166 DIRTY, awaiting their authors. #184 unchanged, take on the desk.

**Absorb — three workers reported this tick (reports appended to #180, #187, #188):** **#180** second pass — `_resolve_kb_names` checks every KB a request names, `/changes` scoped, gate made able to fire (the worker found its own first gate vacuous and widened it; it then found `links.py`'s `source_kb`/`target_kb` outside the resolver → **#186** filed), load sensitivity root-caused to a module-level rate limiter leaking 429s between tests, fail-closed body parse broke four import tests (`UploadFile` consumes the stream — the review's "Starlette caches the body" premise was wrong for streaming handlers) and was fixed by reading only JSON bodies. Suite 4717/0 by the worker; conductor's suite queued, second cold read out. **#187** — six classes routed through `Entry._base_kwargs`, five copies deleted (one more than groomed: `DesignDocEntry`'s inline call), conformance test over `_all_types()`: **RED 59 failed**, not the predicted six — every type subclassing through the five copies was dropping `aliases`/`_schema_version` (all of software-kb, cascade and journalism). One correction to the groom: `task.py`'s copy was already correct. Suite 4501/0; review next tick (no cold read trigger: no server/storage/public shape; the `Unsure` names a test-scoping call on `TaskEntry.metadata`, which the conductor will read). **#188** — wording only, grep gate exactly one `description =` line, zero `.py`/test files; diff read, flipped ready, auto-merge armed.
...
## Tick 2026-09-19T21:38Z (scheduled)

**Merged:** **#187** — priority 2 landed (`dev` 130f433, CI green): every extension entry class routes through `Entry._base_kwargs`, five hand-rolled copies gone, registry-wide conformance test. Worktree and branch cleaned up. With #188 earlier, priorities 2 and 3 are done; priority 1 (#180) is the last of the maintainer's three.

**Absorb — #180 second cold read, one finding reproduced by the conductor:** `POST /api/collections/query-preview` with `{"query": "kb:private-kb"}` returns **200 with private ids, titles and bodies to `anon` and `peer`** — the DSL's `kb:` token sets `query.kb_name`, and the handler uses that to *switch off* the readable filter; the resolver sees only top-level keys and the structural gate is green on the route. Pre-existing on `dev`, so a live leak today, not a regression; but the CHANGELOG, the worker report and the gate all claim the route is covered. Plus: the same hole structurally on the saved-collection path (unproven), the fail-closed 400 is unreachable (FastAPI 422s first) and its test passes on the first-pass head, `_is_kb_parameter` is a heuristic that cannot see a free-text field, repeated `?kb=` params half-inspected. Everything else held: the two-KB and write-tier regressions fail on `1ce02e4` (20/102), `_has_json_body` is a real media-type parse, `SECONDARY_KB_PARAMETERS` complete, `_admin_kb_path_name` is live (keep). Review appended. **Second redispatch → breaker; on the maintainer's desk** — the fourth theme in that state (#140, #145, #161, #180), with the recommendation unchanged: the conductor makes the bounded fixes at review; #180's is a live leak and should go first.

**#173 (outside):** cold read found the push is `9c799d2` rebased — the author's four fixes (`328ffd9`) were never pushed — and on the rebased head the unscoped `_timestamp_same_instant` is a data-loss regression `dev` lacks (an ADR `date` edit reports success and never reaches disk; 78 files in this KB carry such keys), plus two new findings (unparseable `created_at` overwritten with now; tests miss the `KBService.update_entry` path). Conductor reproduced the ADR case and the nine `created_at` shapes. Message drafted for the maintainer's agreement.

**Dispatched — the machine was idle and the two remaining 0.24.2 issues were groomed as spikes on 09-18:** `pyrite-spike` on **#13** (first-write embedding: attach a worker vs record the debt; opus, read-only, 1 slot) and on **#9** (search skeleton: reproduce on packaged and dev-served UI, bundle hash, package-G spec status; opus, the one Playwright-heavy task, 2 slots). Deliverables are ticket comments (and an ADR draft if #13's answer changes `auto_embed`'s promise) — nothing lands on the maintainer's desk from these except a decision if one is needed. 3 of 4 slots.

**Needs the maintainer:** #180/#140/#145/#161 fix-at-review or redispatch; #173 message; #184 comment; the follow-ups note.

## Tick 2026-09-19T23:43Z (scheduled)

**Health:** `dev` 130f433 green. Load 5.8. **Four outside PR events since the last tick:** new **#190** (makiaveli1, "keep response-required fields in REST ?fields= projections" — a second fix for #179, with a shared `pyrite/server/projection.py`, i.e. the follow-up the #184 review planned); **#184** merged `dev` into its branch (no code change; our review was never posted — the take is still on the maintainer's desk); **#175** and **#174** (makiaveli1) both pushed the changes our earlier reviews asked for, gate green. All four claimed; review worktrees at their heads; lint at CI scope clean on all four. Suites `-n 4` running (#190 in slot 1; #175 then #174 in slot 2 — #184's head is its reviewed commit plus a merge, no re-run); cold reads out on #190 (with the #184 comparison and a three-way recommendation for the maintainer's choice between two PRs on one issue), #175 and #174.

**Spikes returned:** **#9** — does not reproduce on four packaged-UI worlds (fresh and stale bundle, auth on/off), the stale-bundle hypothesis is structurally unreachable (the wheel ships no UI; `index.html` is `no-cache`; chunks are content-hashed), and the skeleton/count are mutually exclusive on one store flag. **Not closable yet**: the groom made closing conditional on Playwright package G's `search.spec.ts` asserting the seeded title renders, and G has not been written — `dev`'s spec asserts conditionally. **#13** — design chosen: (b) *record the debt*; (a)'s dilemma rested on a false premise (`EmbeddingWorker` is a SQLite queue with no thread; attaching costs 53 ms), and **nothing in production ever sets `_embedding_worker`**, so every write takes the synchronous branch — one assignment plus a drain closes it. Found on the way: `PyriteConfig.to_dict()` drops `auto_embed`, so a user's `auto_embed: false` is silently reverted by the next `pyrite init`/`kb add` (#43's real defect). #13 and #43 now carry acceptance, touches, regimes, model; #13 waits on #145 (`embedding_service.py`) and packaged-UI theme 1 (`server/api.py`); #43 on #145. **ADR drafted, `proposed`: "Writes are eventually-embedded — `auto_embed` guarantees embedding, not synchrony"** — the spike numbered it 0034 because the log branch predated `dev`'s 0034; the conductor rebased the log branch onto `dev`, renumbered it **ADR-0035**, and relinked both spike comments. Acceptance is the maintainer's.

**Records:** Playwright package F's item closed (#160 merged; the item was never moved). Groom lane dispatched on **package G** (the #9 closure path).
...
## Tick 2026-09-19T23:53Z (absorb: three outside cold reads)

**#175** — the three asks done properly; but the guard the review asked for swapped a loud failure for a quiet one (a non-mapping `metadata:` value is now deleted from the file, and a test pins the deletion), and, worse, a `metadata` key colliding with a known key is now silently dropped on the *create* path — reproduced by the conductor: `GenericEntry(metadata={"status":"draft","title":"shadow"}).to_frontmatter()` loses `"shadow"`; `dev` keeps it. Two small fixes, each with a test. Merged tree with #173 is clean and green. **#174** — both asks done, no 500 on any input, no existence oracle, projection byte-identical to MCP; one CHANGELOG clause. **#190 vs #184** — contradictory contracts: #190 keeps four fields on a rationale its own `JSONResponse` makes moot, leaves MCP and its author's own #174 on two, and conflicts with #184 in both files; recommendation (a): merge #184 with three asks, invite #190 back as the one-helper-for-REST-and-MCP follow-up with a parity test. All three messages drafted (`post/c175-recheck.md`, `c174-recheck.md`, `c184-c190.md`), awaiting the maintainer's agreement. The reviewer's one-off 59-failure on #175 was the #189 artefact (main checkout pulled mid-review), not the PR.

**Desk (with the maintainer present):** fix-at-review yes/no per branch (#180, #161, #145, #140); #184/#190; #174; #175; #173; ADR-0035; follow-ups. Running: package G worker (#191).

## Tick 2026-09-19T23:53Z (scheduled — quiet)

`dev` 130f433 green. No outside activity since the last tick; no new PRs or issues. The package-G worker (#191) is mid-run (one server, one Playwright process alive; load 2.8, 65% free). Every outside PR has a review done and a message drafted; every loop branch is on the desk. Nothing to absorb, nothing dispatchable within the budgets.

## Retro 6 — 2026-09-20T00:30Z — window 2026-09-18T10:48Z → 2026-09-20T00:25Z (21 ticks; 5 loop themes + 2 outside PRs landed; 4 breaker trips; one incident)

### What worked
- **`dev` stayed green: 11 of 11 CI runs on `dev` in the window `success`** (one `cancelled` by the concurrency rule), gate 1–5 min. Nothing broken landed.
- **The cold read changed every PR it touched: 12 of 12.** #180 (×2), #161 (×2), #145, #140, #173 (×2), #184, #190, #174, #175 — and the conductor reproduced 15 findings before acting; none was wrong. Two would have merged silent corruption on the maintainer's say-so (#173's rebased head: an ADR `date` edit never reaching disk; #175's create path dropping a colliding `metadata` key), and one is a live private-KB leak on `dev` (#180's `query-preview`).
- **The two budgets held under real load.** Three worker suites overlapped (load 14.9) and memory stayed ≥60% free; the conductor's own suites queued behind a `< 7` gate three times instead of piling on. The #168 failure did not recur.
- **Outside contributions: 5 messages posted with the maintainer's agreement, 2 merged (#171, #176), 6 more reviewed and answered** — 7 first-time or repeat contributors, one message each. The "agree before posting" rule cost one round trip per batch and produced zero corrections.
- **Spikes as the unit for unknowns: 2 of 2 changed their tickets.** #13 (design chosen; "nothing in production ever sets `_embedding_worker`"; ADR-0035 drafted) and #9 (not reproducible across four packaged-UI worlds; closure path = package G, now dispatched).
- **Lead time when the desk was clear: #187 43 min, #188 14 min** claim → merged, both Sonnet.

### Failures and root causes
- **OOM incident (#168, 2026-09-18 11:10Z; 8 concurrent `-n auto` suites)** → the cap counted dispatched workers, not suites (conductor's own, reviewers told to run the suite, pre-push inside each worker) → the machine budget in `SKILL.md` (PR #183) counts suite slots and pins `-n 4`. Closed; verified under load this window.
- **Four breaker trips (#140, #145, #161, #180)** → each second cold read found real things → but not the first finding again: the surface the first fix *widened* (pull/push now on the regex; the `k` cap clamped into silent under-return; the resolver complete but blind to a DSL) → the redispatch spec named the fixes, not the surface they make load-bearing; and the breaker sent bounded lists to the desk → `SKILL.md` breaker amendment (this retro's change); the "name the widened surface" line is the next retro's if trips continue.
- **False `59 failed` on #187, then #190/#175/#174 (#189)** → the conductor's review worktrees symlinked the main checkout's `.venv` → editable extension installs import from the main checkout, which was three commits behind `dev` → `scripts/new-worktree.sh` already says so at lines 38–39; the shortcut was the conductor's → #189 filed: `review.md` says why the script is mandatory; `verify-red.sh` should exit 2 when a reverted file's package resolves outside the worktree. Interim: keep the main checkout on `origin/dev` at every health step (done).
- **ADR number collision (spike numbered its ADR 0034; `dev` had 0034)** → the log branch predated `dev`'s 0034 → `pyrite sw new-adr` numbers from the branch's `kb/`, not from `dev` → renumbered to 0035 by hand; the tool gap is a one-liner on the follow-ups note (number from `origin/dev`'s ADR list).
- **The retro did not run for 38 hours** despite the cadence rule ("immediately after a breaker trip"; a day is the floor) → the retro cron was session-bound and the session died in the OOM; the resumed conductor loop re-armed itself but not the retro → noted below, not this retro's change (one change per retro).
- **`pyrite create -t note` created an `adr`** (architect, 2026-09-19) → same class as `bug-create-silently-accepts-undeclared-types` → product bug, filed #197 (the follow-ups note went out as #192–#196 on the maintainer's word).
- **#173's author reported four fixes "in `328ffd9`" that were never pushed** → the reviewer trusted the reply until the cold read diffed the heads → not a process failure — the cold-read-always rule caught it — but the outside-PR checklist should diff the new head against the last reviewed head *before* reading the author's reply (a line in `review.md`, folded into the #189 change).

### Constraint
**The maintainer's desk.** Evidence: the tick log's "Needs the maintainer" section carried the same four loop branches for **12 consecutive ticks** (~5 h); five drafted contributor messages waited ~4 h; the loop's own build lane dispatched nothing but spikes and one Playwright package for the last 8 ticks because every ready theme was behind a held branch (#145 gates #13 and #43). Outside PR age at the tick: 1–37 h, median 14 h. The review lane produced verdicts faster than the desk could absorb them: 12 reviews delivered, 0 loop branches landed from them. Per the skill, the remedy is to reduce what reaches the desk — hence the breaker amendment, not more dispatch.

### Waste, in the seven
- **Delays (dominant):** the desk queue above; plus #160/#163's claim → merged of **34 h** each — dispatched 09-18 10:2x, held first by WIP=1 after the OOM, then by the "land nothing while an outside PR awaits the maintainer's click" rule while #171 sat `action_required` → both correct rules, and both were the loop choosing the maintainer's click over its own throughput; the rule stays, the number is recorded.
- **Handoffs:** four second passes whose spec did not name the widened surface; #173's unpushed commit.
- **Defects:** none reached `dev`; two near-misses caught (#173, #175), one pre-existing leak found (#180).
- **Relearning:** the venv trap, four times before the cause was found (#189); the ADR numbering.
- **Partially done work:** package F's item sat `in_progress` after #160 merged (closed this window); Playwright package G existed groomed since 09-18 under a different title and was nearly re-created — the board is the source of truth only if it is searched by id, not title.
- **Extra features / task switching:** none seen — footprints matched Touches on all five landed themes; ticks stayed bounded (longest ~25 min).

### Friction observed
- The conductor had to reproduce every reviewer finding before acting (15 times) — right, and cheap (2–6 min each; the one 12-minute case was a probe that accidentally imported and re-ran a 405-test module). Keep.
- The conductor had to rebuild a venv per review worktree after discovering #189, and re-run three conformance files by hand — the script exists; the checklist did not say to use it.
- The architect had to detect an existing groomed item by content because the title differed — `pyrite search` finds it; a title grep did not.
- The maintainer had to ask "tell me more about the breaker" — the rule's *why* was in the skill, but the tick reports said only "breaker tripped"; the next report should carry the one-paragraph rationale the first time it trips in a session.
- A `gh issue create --body "$(cat <<EOF …)"` with backticks failed on shell parsing (the same class as #123); `--body-file` every time.

### The one process change
**A bounded, reproduced second send-back is a fix at review, not a breaker trip** — `SKILL.md` "Stop conditions", with the numbers above; the breaker keeps tripping on design-shaped or third-pass findings, on red `dev`, and on reverts. **No PR: the edit to `.claude/skills/pyrite-conductor/SKILL.md` was refused by the session's self-modification guard** (the meta-conductor may not rewrite the skill that governs the running conductor from inside the session). It moves a line of the delegation boundary (the breaker's "do not dispatch again until the maintainer says so"), so it is a **recommendation** for the maintainer to apply or decline; the four held branches are its first application if accepted. The paragraph to append after the breaker paragraph in "Stop conditions":

> **The bounded second send-back is not a breaker trip — it is a fix at review.** When a second cold read on a theme returns findings that are (a) each reproduced by the conductor, (b) each a stated, local change (a few lines, one test), and (c) none a design decision or a widening of the theme, the conductor applies them itself in its review worktree — one commit per finding, `fix:` commits touching `tests/`, a cold read on the delta only — and flips the PR under the ordinary budgets. The breaker still trips when any finding is design-shaped, unbounded, or would be the theme's *third* pass, and always on a red `dev` push or a revert. Retro 6 (2026-09-20): four themes (#140, #145, #161, #180) tripped the old rule in one window; every one had a bounded, reproduced list, and every one waited on the maintainer's desk for hours while the loop dispatched nothing — the desk, not the worker, was the constraint. The second cold reads were not finding the first finding again; they were finding the surface the first fix *widened* (pull/push routed through the regex; the `k` cap clamped; the resolver made complete), which is exactly the class a fix-at-review closes and a third worker pass tends to widen further. The tick log records each fix-at-review with the findings and the delta cold read's verdict, so the retro can see whether the exception is being stretched.

### The quality theme
**One shared frontmatter-split function; retire 8+ hand-rolled copies** (`shared-frontmatter-split-utility`, now tagged `quality` with a `## Groom 2026-09-20`) — the same duplication class #187 just fixed for `_base_kwargs` (five copies, 59 red types), one layer down: eight copies of `split("---", 2)` that answer differently on a body containing `---`, CRLF, a BOM, a missing trailing newline. Footprint: nine call sites, `pyrite/utils/yaml.py`, a conformance corpus; sequence after #173/#175 (both edit the round-trip tests). Sonnet; cold read yes (storage + file format). Code health otherwise: suite 4501 passed in 85 s at `-n 4`, slowest test 5.2 s, ruff clean at CI scope — no slow-test theme is warranted.

### Expected effect
Desk-held loop branches from **4 (for ≥5 h)** to **0** within two ticks of the amendment landing, and time-to-flip for a second-pass theme from "until the maintainer reads the tick" to **≤1 tick** (fix + delta cold read). Measured at retro 7: breaker trips per five landed themes from 4 to ≤1; fix-at-review count and each delta cold read's verdict in the log. **Revert if** a fix-at-review's delta cold read returns a *new* finding on two themes in a row (the exception is being used to absorb design work), or if any fix-at-review lands red.

### Not changed, noted for next time
- **Retro cadence survives a session crash** — the retro was 38 h late. Fix lives in the conductor's resume step: re-arm the retro cron alongside the tick cron. One line; next retro's change if the desk change is taken now.
- **Redispatch template names the widened surface** (`dispatch.md`) — the root cause of the four second-pass findings; deferred so this retro makes one change.
- **`review.md`: diff the new head against the last reviewed head before reading an author's reply** (#173) and **use `scripts/new-worktree.sh` for review worktrees** (#189) — both belong in the #189 fix.
...
## Tick 2026-09-20T00:23Z (backlog groom returned; #49 dispatched; #131 spike)

**Merged:** #191 (package G) → `dev` 24533f4; #9 closed on the merge, evidence recorded on the issue.

**Backlog groom (maintainer's request, two architects, read-only, 1h):** every open issue now carries a dated groom or triage. **`good first issue` pool: 14, all verified against `dev` 130f433** — #44, #67, #68, #95, #192, #197, #196, #7, #8, #63, #47, #65, #12, #20 — each with verbatim error, reproduction, acceptance, files with line numbers, regimes; none security-adjacent, none in a milestone, none overlapping an open PR. Four labels removed as claimed (#179→#184/#190, #134→#174, #19→#177, #58→#166); #96 does not reproduce (recommend close); #6/#22 kept out (M/opus); two splits filed (#199 server logging, #200 discover-neighbors stubs, both `needs-design`). The other 62: 31 ready with model/heavy/cold-read, 3 claimed, 13 recommended closures (10 `process` issues already fixed by merged skill changes: #80 #91 #101 #109 #111 #114 #119 #123 #104 #130; #107 superseded by #122), 6 needing the maintainer (#10, #59, #135 → `needs-design`; #54 §1; #193 where the helper lives; ADR-0035 before #13), 5 spikes (#131 first — blocks package H), and file-contention chains written down (`base.py`: #90→#178→#148; the 8 search files: #145→#152→#62; scoping: #180→#186→#201). **New #201:** MCP over HTTP has no per-KB read scoping (read scoping part 3). Found on the way: the #13/#43 spike comments had posted as the literal text `@/tmp/c13.md` (a dropped `--body-file`); the text survived and was reposted inside the triage comments; placeholders deleted; read-back rule noted on #123.
...
## Tick 2026-09-20T01:04Z (absorb: #202 sent back; #203 in review)

**#202 (#49) — sent back (first redispatch), one finding reproduced by the conductor:** a same-route navigation (`entries/+page.svelte:68` does `goto(url, {replaceState: true})` on every filter change) leaves `document.title` equal to the `beginNavigation` snapshot because the leaf's static `<title>` effect does not re-run, so the writer decides "unclaimed" and writes `Pyrite` — #49 reintroduced on one of its own five routes. The design (string-diffing `document.title` against a snapshot) cannot distinguish "re-claimed the same string" from "claimed nothing". Two more: the branding effect firing mid-navigation consumes the snapshot; `entries/[id]` renders its title inside `{#if seo}` so every entry open flashes the brand. The tests never render a component, so none could see it; making `beginNavigation` a no-op fails one test of nine. The worker's diagnosis was right and its reason wrong (Svelte compiles `<title>` to `document.title = …` in an effect; last effect wins; no shared element). **Redispatched with the shape the cold read verified against all four regimes:** a `{#if UNTITLED_ROUTES.includes($page.route.id)}` guard in the layout head (three lines, no hooks), a structural test that the list equals the routes without a `<title>`, a `@testing-library/svelte` component test incl. same-route re-render, and the three e2e `toHaveTitle` assertions restored with one Playwright run. The idiomatic single-source `$page.data.title` shape is filed as a `quality` follow-up item (`one-title-in-the-app-page-titles-come-from-load-data…`).
...
## Tick 2026-09-20T01:20Z (maintainer: process findings move to the KB)

Maintainer (2026-09-20): "fix all the process issues, and keep those in the pyrite kb rather than in github"; the CONTRIBUTING claim-by-plan idea approved ("that might make the response to 198 even cleaner"). Done this tick: **12 GitHub issues closed** with the fix named in each (#80 #91 #101 #104 #109 #111 #114 #119 #123 — fixed by merged skill/script changes; #107 superseded by #122; #130 fixed by #138; #96 does not reproduce). **Five still-open process findings migrated** into `kb/backlog/` items tagged `process` with their triage text (#189 → `review-worktrees-that-symlink…`, #133 → `the-pr-gate-runs-one-interpreter…`, #122 → `claude-theme-md-is-gitignored…`, #115 → `a-well-named-test-over-an-uncovered-case…`, #103 → `changelog-md-is-the-one-file…`) and the GitHub issues closed pointing at the ids; #182 (the org move, the maintainer's own task) stays. **ADR-0033 amendment drafted `proposed`**: process findings are KB items; GitHub keeps bugs, requests and the maintainer's tasks; the retro reads `pyrite sw backlog` filtered on `process`. **Dispatched (Sonnet ×2):** `process/open-process-items` — #122 (`git rm --cached .claude/THEME.md` + a tracked-and-ignored test), #189 (`verify-red.sh` exits 2 when a reverted file's package resolves outside the worktree, + test; the `review.md` line if the permission system allows it), #133 (`ci.yml` runs the full interpreter matrix when a PR touches test infrastructure, pinned by a test); `docs/contributing-claim-by-plan` — CONTRIBUTING's claiming rule (plan on the ticket or a draft PR; 5-day lapse; first PR meeting acceptance merges, the other credited; placeholders are not claims; keep work pushed), plus the issue/PR templates if they exist. #115 (the cold-read Unsure tension) is a one-line skill decision — the maintainer's. #103 is the groomed `changelog-fragments` quality item, after #140. Slots: 2 workers + #203 cold read's server = 3 of 4.

## Tick 2026-09-20T01:40Z (session wound down by the maintainer)

**The maintainer's desk exists.** `desk/` at the repo root is a gitignored Pyrite KB (`pyrite-desk`; schema and how-to in #205: types `task`, `conductor_tick`, `retro`, `note`, every entry with a `project`), seeded with the 20 decisions on the desk, tagged, each with a link and a one-line recommendation — `pyrite task list -k pyrite-desk --status open`. A second desk for the journalism research project at `tcp-kb-internal/desk` (`journalism-desk`). **From the next session the tick log and retros live in the desk, project-scoped; this repo log ends here with a pointer.** Landed since the last entry: #204 (CONTRIBUTING claim-by-plan rule). Armed: #205 (desk/ + docs), #206 (three process fixes: THEME.md untracked, `verify-red.sh` refuses a suite number from a tree the interpreter is not importing — smoke-tested by the conductor against a real symlinked-venv worktree — and the PR matrix widens when a diff touches test infrastructure; one review.md bullet, disclosed in the PR). **#203 sent back** (review on the PR: `_app_get_db` is now a generator but still passed to `mount_mcp_routes` → 500 with auth; fallback sessions leak a pooled connection per thread; singletons take a second connection so 40 threads exhaust the pool; the isolation mechanism itself is sound and the headline reproduces) — first redispatch not dispatched before the close. **#202 second pass** reported, not reviewed. Cron stopped. Resume: `/pyrite-conductor` in a fresh session; `desk/notes/how-to-resume-2026-09-20.md`.
