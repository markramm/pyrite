...
def parse_datetime(s: Any) -> datetime:
    """Parse a frontmatter timestamp.

    Accepted forms:

    * ``datetime`` -- returned unchanged (tz offset, if any, kept).
    * ``datetime.date`` -- a bare YAML date (``created_at: 2026-01-15``) is
      loaded by the YAML parser as a ``date``, not a string. Anchored to
      midnight **UTC**: every timestamp the model *creates* is UTC
      (``_utcnow``), so a file-authored date must not become "now". Before
      this branch existed the value fell through to the fallback and every
      bare date in a file read back as the load time (#151).
    * ISO-8601 ``str`` -- a trailing ``Z`` is treated as UTC; a string with
      no offset is kept naive, as before.
    * anything else (missing/empty) -- "now" in UTC.
    """
